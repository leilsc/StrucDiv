#' @name Nested_StrucDiv
#' @rdname Nested_StrucDiv
...
Nested_StrucDiv <- function(x, wsl, WSLw, dist = 1, angle = "all",
  ...
  if (raster::canProcessInMemory(out)) {
    ...
    suppressMessages( vMat_big <- .getValuesWindow(x, wsl = WSLw, padValue = padValue, 
                                 aroundTheGlobe = aroundTheGlobe)  )
    ...
    switch_angle <- function(angle) {
      ...
      switch(angle,
             ...
             "all" = .ProbabilityMatrixAllNested(vMat = vMat, 
                                                 vMat_big = vMat_big,
                                                 d = dist, narm = narm),
             .ProbabilityMatrixHorizontalNested(vMat = vMat,                                                                  vMat_big = vMat_big,
                                                vMat_big = vMat_big,
                                                d = dist, narm = narm)
      )
