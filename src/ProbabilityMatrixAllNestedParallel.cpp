...
// [[Rcpp::export(name = ".ProbabilityMatrixAllNestedParallel")]]
NumericMatrix ProbabilityMatrixAllNestedParallel(NumericVector vMat, NumericVector vMat_big,
                                int d, bool display_progress=false){
  
  // NumericVector outList(vMat.nrow());
  
  // NewProgressBar pb;
  
  // Progress p(vMat.nrow()*vMat.nrow(), display_progress, pb);
  
  int bb = sqrt(vMat.length());
  int bbb = sqrt(vMat_big.length());
  
  int AllPairs = ( 2*( bb*(bb - d) + bb*(bb - d) +
                   2*(bb - d)*(bb - d) ) ) +
                   ( 2*( bbb*(bbb - d) + bbb*(bbb - d) +
                   2*(bbb - d)*(bbb - d) ) );
  
    NumericVector rowvec = vMat;
    NumericMatrix xMat2( sqrt(vMat.length()), sqrt(vMat.length()), rowvec.begin() );
    ...
    NumericVector rowvec_big = vMat_big;
    NumericMatrix xMat2_big( sqrt(vMat_big.length()), sqrt(vMat_big.length()), rowvec_big.begin() );
    ...
    // NumericMatrix bla( 2 );
    // CharacterVector character = {"num","num"};
    // colnames(bla) = character;
    // rownames(bla) = character;
    
    if( (Values.length() == 0) | (Values_big.length() == 0) ) {
      return NA_REAL;}
    ...
    NumericMatrix outList = out2_big2;
