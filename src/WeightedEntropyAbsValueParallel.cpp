...
// [[Rcpp::export(name=".WeightedEntropyAbsValueParallel")]]
double WeightedEntropyAbsValueParallel( NumericVector Hetx, NumericMatrix PMat,
                                        bool narm,
      ...
      return NA_REAL;
        ...
        for (int m = 0; m < xPMatSub.nrow(); m++) {
          for( int n = 0; n < xPMatSub.ncol(); n++) {
            ...
            else EntMat(m,n) = std::abs( xVal[m] - xVal[n] )*( xPMatSub(m,n) * (-log(xPMatSub(m,n))) );
            
          }
        }
        
        double out = sum(EntMat);
        return(out);
